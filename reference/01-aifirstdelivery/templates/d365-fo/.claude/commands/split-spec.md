...
1. Read `docs/{requirement-name}/fdd.md`. If the file does not exist, stop: "No FDD found. Run /fdd first."
...
- [ ] F&O agent: run `/fdd-review {requirement-name}` to validate the F&O FDD
...
F&O Next Step            : /fdd-review {requirement-name}
...
- F&O FDD retains the original file path — downstream commands (`/fdd-review`, `/tdd`, `/plan`) do not need to change paths.
