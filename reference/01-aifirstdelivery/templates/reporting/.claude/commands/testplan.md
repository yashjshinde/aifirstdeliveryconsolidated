...
2. If status is not `APPROVED`, stop: "Run /review first. Test planning requires an approved spec."
...
5. Read `plans/{feature-name}/plan.md` if it exists — extract User Story references. If not, set all US-NNN fields to `*(pending — run /plan first)*`.
