...
**YAML front matter** — set `status: DRAFT`. Do not change field names; `/review` reads them.
...
- If `/spec-alm` was used (structured intake): also include the L1/L2/L3 ALM IDs from the intake document, linking each FR-NNN to its parent Epic, Feature, and User Story IDs.
