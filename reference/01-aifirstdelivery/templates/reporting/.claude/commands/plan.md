...
1. Read `specs/{feature-name}/review.md`. If status is not `APPROVED`, stop: "Run /review first."
...
3a. Check `brownfield.enabled` in `constitution/10-alm-configuration.md`.
    ...
    - Read `specs/{feature-name}/impact-analysis.md` — **required**; if it does not exist, stop:
      "Run /impact {feature-name} first — impact-analysis.md is required by /plan when brownfield mode is enabled."
