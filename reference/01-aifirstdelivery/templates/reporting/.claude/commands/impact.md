...
3. Read `{brownfield.docs-path}/component-inventory.md`.
   If it does not exist, stop: "Brownfield docs not found at `{brownfield.docs-path}`. Run the CE Brownfield agent's `/generate` command first to produce the documentation set, then re-run /impact."
