...
## Ready Tasks
{List of task IDs that are fully ready for /task generation}

## Next Step
{If TASK-READY: "Run /task {feature-name} to generate task cards"}
{If PARTIALLY READY: "Run /task {feature-name} — tasks with QUESTION flags will be included with warnings"}
{If NOT READY: "Resolve BLOCKERs in plan.md then re-run /clarify {feature-name}"}
```
