...
Sets the FACTORS APPROVED gate required by `/estimate-build` when new factors are pending.

## Usage

```
/factors-review {project}
```

## Pre-condition Check

1. Check `estimates/{project}/proposed-factors.md` exists.
   If not: "No proposed factors found for project '{project}'. Run `/estimate-rom`, `/estimate-spec`, or `/estimate-plan` first."
...
6. Resolution:
   ...
   - If any factor remains REJECTED and cannot be remapped:
     ...
     - Print: "BLOCKED: {n} factor(s) were rejected. Update `estimates/{project}/proposed-factors.md` with revised definitions and re-run `/factors-review {project}`."
