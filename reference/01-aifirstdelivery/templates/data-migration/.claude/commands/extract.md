...
/extract testsuites {migration-id} [suite-name]
/extract testcases  {migration-id} [tc-id]
```
...
7. Print the completion report:
   ...
   Next step: /alm push-tests {m}  (creates test plan in ADO) or /alm sync-tests {m} (after manual import)
   ```
