...
3. **Multi-Domain Detection — run first, before any other check.**
   ...
   - **BLOCKER text (CE / Power Apps):** "This spec contains D365 CE or Power Apps requirements that are out of scope for the Data Migration agent. Run `/split-spec {migration-id}` to separate the data migration and CE/Power Apps portions before proceeding. The CE/Power Apps spec must then be handed to the appropriate domain agent."
   - **BLOCKER text (Integration):** "This spec contains Azure Integration requirements that are out of scope for the Data Migration agent. Run `/split-spec {migration-id}` to separate the data migration and integration portions before proceeding. The integration spec must then be handed to the Azure Integration agent."
...
- If a CONFLICT is detected (two migrations targeting the same staging table or Dataverse entity with conflicting logic): raise as WARN — "Component {name} is also claimed by plan {other-plan} with action {action}. Resolve before /plan to avoid conflicts."
...
**APPROVED** — spec is complete and meets all constitution requirements. Proceed to /mapping and /pipeline.
...
{If APPROVED:}
Next steps:
  /mapping {migration-id}     → field mapping document
  /pipeline {migration-id}    → pipeline design document
  /testplan {migration-id}    → test plan

{If CHANGES REQUIRED:}
Fix the listed issues and re-run /review {migration-id}.
```

**GATE:** No downstream command (`/mapping`, `/pipeline`, `/testplan`, `/plan`) may run until this review is APPROVED.
