...
1. Read `specs/{migration-id}/spec.md`. If the file does not exist, stop: "No spec found. Run /spec first."
...
- [ ] Data Migration agent: run `/review {migration-id}` to validate the migration spec
- [ ] CE agent: copy `specs/{migration-id}-ce/` and run `/review {migration-id}-ce`
- [ ] Integration agent: copy `specs/{migration-id}-integration/` and run `/review {migration-id}-integration`
- [ ] Reporting agent: copy `specs/{migration-id}-reporting/` and run `/review {migration-id}-reporting`
- [ ] Ensure cross-cutting MR dependencies are reflected in all plans when `/plan` is run
```
...
Migration Next Step      : /review {migration-id}
CE Next Step             : /review {migration-id}-ce            ← run in the D365 CE agent
Integration Next Step    : /review {migration-id}-integration   ← run in the Integration agent
Reporting Next Step      : /review {migration-id}-reporting     ← run in the Reporting agent
```
...
- Migration spec retains the original file path — downstream commands (`/review`, `/mapping`, `/pipeline`, `/plan`) do not need to change paths.
