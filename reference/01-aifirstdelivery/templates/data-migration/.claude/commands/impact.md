...
Run `/impact` when migrating data into a Dataverse environment (or other target) that already contains data.
Skip this command for greenfield targets or for one-way export flows (Dataverse → SFTP).

## Steps

1. Read ALL files in `constitution/`.
2. Verify brownfield is enabled — if not, print: "Brownfield mode is disabled. `/impact` is not required. Proceed to `/plan`."
...
- [ ] Export target entity to CSV via `/implement` export variant before first ingest run
...
The analysis is IMPACT-ASSESSED when the approver writes `IMPACT-ASSESSED` explicitly at the bottom of the document (or confirms via `/review` of the impact analysis).
...
Once IMPACT-ASSESSED, proceed to /plan {migration-id}.
```
