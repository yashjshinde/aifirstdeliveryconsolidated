...
# Spec Review — Asset Management - Customer Asset 360 View
...
| Spec File | `templates/d365-ce/specs/f485-asset-management-customer-asset-360-view/spec.md` |
| Parent Epic | 454 |
| Feature ALM ID | 485 |
...
| Field Service entity context | FS context: msdyn_workorder, msdyn_customerasset, msdyn_functionallocation, msdyn_agreement, work order, … |
...
| RC-003 | Review against FS-specific rules (constitution/12, /13) | §5 / §6 | Spec references Field Service entities (msdyn_workorder, msdyn_customerasset, msdyn_functionallocation, msdyn_agreement, work order, functional location, customer asset). Apply FS-specific rules at /plan: extend OOB tables only, never replace; mobile offline ceilings (≤15 linked tables, <1 GB DB); customer-facing notifications use Account.PreferredLanguageCode. |
...
|---|---|---|---|
| QS-001 | FR-NNN are intentionally deferred to /plan per intake mode (l3-intake: optional). Confirm reviewer accepts this pre-FR state for /review approval, or set l3-intake: required and re-run /spec-alm. | YAML front matter (l3-intake) | If reviewer wants FRs at /review time, this spec must be regenerated after adding L3 PBIs in ADO under Feature 485. |
...
- Approved at the spec level. Resolve QUESTIONs (especially QS-001 — confirm intake mode) before running `/plan f485-asset-management-customer-asset-360-view`.
- Suggested `/plan` will create the pending L3 (Product Backlog Item) for Feature 485, generate FR-NNN from it, and emit Tasks under the L3.
