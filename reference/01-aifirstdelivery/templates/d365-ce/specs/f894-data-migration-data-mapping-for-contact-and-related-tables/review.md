...
# Spec Review — Data Migration - Data Mapping for - Contact and related tables
...
| Spec File | `templates/d365-ce/specs/f894-data-migration-data-mapping-for-contact-and-related-tables/spec.md` |
| Parent Epic | 460 |
| Feature ALM ID | 894 |
...
|---|---|---|---|---|
| BL-002 | Multi-Domain — Data Migration signals detected (constitution/14) | Title / §5 / §11 | Spec contains Data Migration signals: adf, sftp, data migration, data mapping | Run /split-spec f894-data-migration-data-mapping-for-contact-and-related-tables to separate the CE and data-migration portions; the data-migration spec must be handed to the Data Migration agent. The d365-ce slice keeps any Dataverse schema/form work; ADF/SFTP/mapping moves to data-migration. |
...
|---|---|---|---|
| QS-001 | FR-NNN are intentionally deferred to /plan per intake mode (l3-intake: optional). Confirm reviewer accepts this pre-FR state for /review approval, or set l3-intake: required and re-run /spec-alm. | YAML front matter (l3-intake) | If reviewer wants FRs at /review time, this spec must be regenerated after adding L3 PBIs in ADO under Feature 894. |
...
- **BLOCKER:** Run `/split-spec f894-data-migration-data-mapping-for-contact-and-related-tables` to separate domain-mismatched portions, then re-run `/review f894-data-migration-data-mapping-for-contact-and-related-tables` on each split.
- `/plan f894-data-migration-data-mapping-for-contact-and-related-tables` will refuse to proceed while the verdict is NEEDS REWORK.
