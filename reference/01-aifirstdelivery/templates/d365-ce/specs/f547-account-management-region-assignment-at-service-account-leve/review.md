...
# Spec Review — Account Management - Region Assignment at service Account Level
...
| Spec File | `templates/d365-ce/specs/f547-account-management-region-assignment-at-service-account-leve/spec.md` |
| Parent Epic | 454 |
| Feature ALM ID | 547 |
...
|---|---|---|---|
| QS-001 | FR-NNN are intentionally deferred to /plan per intake mode (l3-intake: optional). Confirm reviewer accepts this pre-FR state for /review approval, or set l3-intake: required and re-run /spec-alm. | YAML front matter (l3-intake) | If reviewer wants FRs at /review time, this spec must be regenerated after adding L3 PBIs in ADO under Feature 547. |
...
- Approved at the spec level. Resolve QUESTIONs (especially QS-001 — confirm intake mode) before running `/plan f547-account-management-region-assignment-at-service-account-leve`.
- Suggested `/plan` will create the pending L3 (Product Backlog Item) for Feature 547, generate FR-NNN from it, and emit Tasks under the L3.
