...
# Spec Review — Data Migration - Data Mapping for - Account and related tables
...
| Spec File | `templates/d365-ce/specs/f893-data-migration-data-mapping-for-account-and-related-tables/spec.md` |
| Parent Epic | 460 |
| Feature ALM ID | 893 |
...
|---|---|---|---|
| QS-001 | FR-NNN are intentionally deferred to /plan per intake mode (l3-intake: optional). Confirm reviewer accepts this pre-FR state for /review approval, or set l3-intake: required and re-run /spec-alm. | YAML front matter (l3-intake) | If reviewer wants FRs at /review time, this spec must be regenerated after adding L3 PBIs in ADO under Feature 893. |
...
- **BLOCKER:** Run `/split-spec f893-data-migration-data-mapping-for-account-and-related-tables` to separate domain-mismatched portions, then re-run `/review f893-data-migration-data-mapping-for-account-and-related-tables` on each split.
- `/plan f893-data-migration-data-mapping-for-account-and-related-tables` will refuse to proceed while the verdict is NEEDS REWORK.
