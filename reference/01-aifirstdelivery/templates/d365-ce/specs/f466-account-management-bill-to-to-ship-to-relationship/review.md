...
# Spec Review — Account Management - Bill‑To to Ship‑To Relationship
...
| Spec File | `templates/d365-ce/specs/f466-account-management-bill-to-to-ship-to-relationship/spec.md` |
| Parent Epic | 454 |
| Feature ALM ID | 466 |
...
| Field Service entity context | FS context: msdyn_customerasset, msdyn_functionallocation, functional location, customer asset |
...
| RC-003 | Review against FS-specific rules (constitution/12, /13) | §5 / §6 | Spec references Field Service entities (msdyn_customerasset, msdyn_functionallocation, functional location, customer asset). Apply FS-specific rules at /plan: extend OOB tables only, never replace; mobile offline ceilings (≤15 linked tables, <1 GB DB); customer-facing notifications use Account.PreferredLanguageCode. |
...
|---|---|---|---|
| QS-001 | FR-NNN are intentionally deferred to /plan per intake mode (l3-intake: optional). Confirm reviewer accepts this pre-FR state for /review approval, or set l3-intake: required and re-run /spec-alm. | YAML front matter (l3-intake) | If reviewer wants FRs at /review time, this spec must be regenerated after adding L3 PBIs in ADO under Feature 466. |
...
- Approved at the spec level. Resolve QUESTIONs (especially QS-001 — confirm intake mode) before running `/plan f466-account-management-bill-to-to-ship-to-relationship`.
- Suggested `/plan` will create the pending L3 (Product Backlog Item) for Feature 466, generate FR-NNN from it, and emit Tasks under the L3.
