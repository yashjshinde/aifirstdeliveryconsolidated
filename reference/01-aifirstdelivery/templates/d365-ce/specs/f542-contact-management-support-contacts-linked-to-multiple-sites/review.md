...
# Spec Review — Contact Management- Support Contacts Linked to Multiple Sites
...
| Spec File | `templates/d365-ce/specs/f542-contact-management-support-contacts-linked-to-multiple-sites/spec.md` |
| Parent Epic | 454 |
| Feature ALM ID | 542 |
...
| Field Service entity context | FS context: msdyn_functionallocation, functional location |
...
| RC-003 | Review against FS-specific rules (constitution/12, /13) | §5 / §6 | Spec references Field Service entities (msdyn_functionallocation, functional location). Apply FS-specific rules at /plan: extend OOB tables only, never replace; mobile offline ceilings (≤15 linked tables, <1 GB DB); customer-facing notifications use Account.PreferredLanguageCode. |
...
|---|---|---|---|
| QS-001 | FR-NNN are intentionally deferred to /plan per intake mode (l3-intake: optional). Confirm reviewer accepts this pre-FR state for /review approval, or set l3-intake: required and re-run /spec-alm. | YAML front matter (l3-intake) | If reviewer wants FRs at /review time, this spec must be regenerated after adding L3 PBIs in ADO under Feature 542. |
...
- Approved at the spec level. Resolve QUESTIONs (especially QS-001 — confirm intake mode) before running `/plan f542-contact-management-support-contacts-linked-to-multiple-sites`.
- Suggested `/plan` will create the pending L3 (Product Backlog Item) for Feature 542, generate FR-NNN from it, and emit Tasks under the L3.
