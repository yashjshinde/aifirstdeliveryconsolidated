...
- `/validate {feature-name}` — validates ALL task cards for a feature
- `/validate {feature-name}/{task-file}` — validates a single task card
...
- `READY TO IMPLEMENT` — all checks pass; `/implement` may proceed
...
ACTION: Fix issues in {L4-Prefix}-002, then re-run /validate before running /implement.
```

## Rules

- `/implement` will refuse to execute a task unless its `validation-status` is `READY TO IMPLEMENT`
- Re-run `/validate` after fixing a task card — it will re-check and update the status
