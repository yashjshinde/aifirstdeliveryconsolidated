...
parent-epic-id: "454"
feature-alm-id: "465"
plan-ref: plans/f465-account-management-informational-parent-hierarchy-in-d365/plan.md
spec-ref: specs/f465-account-management-informational-parent-hierarchy-in-d365/spec.md
---

# Plan Clarification Report — Account Management - Informational Parent Hierarchy in D365
...
|---|---|---|---|
| T-001 | Add custom Parent Account lookup configuration on Account form | QUESTION | See Questions section |
| T-002 | Create custom security role `opex_OperationsAdmin` | READY | — |
| T-003 | Configure column-level audit for Parent Account | READY | — |
...
| T-007 | Add custom Parent Account lookup configuration on Account form | QUESTION | See Questions section |
| T-008 | Create custom security role `opex_OperationsAdmin` | READY | — |
| T-009 | Configure column-level audit for Parent Account | READY | — |
...
| QS-002 | T-004 | supported-languages list (per `constitution/10-alm-configuration.md`) currently is en-US only. If multi-language is intended at first ship, confirm the language set before generating RESX scaffolding. | Multi-language-ready posture (Constitution §15) — generate RESX skeleton for en-US only at /implement; additional languages added later as a config event. |
| QS-003 | T-005 | supported-languages list (per `constitution/10-alm-configuration.md`) currently is en-US only. If multi-language is intended at first ship, confirm the language set before generating RESX scaffolding. | Multi-language-ready posture (Constitution §15) — generate RESX skeleton for en-US only at /implement; additional languages added later as a config event. |
| QS-004 | T-006 | Test environment readiness — does Test environment have upstream Fusion → D365 sync data populated to a representative volume? | Test data fixture (per Test Plan §6) is loaded; Fusion sync features 463/464/546/575 are live in Test. |
| QS-005 | T-007 | Form / view layout requires OPEX wireframe sign-off (per Constitution §10 CR-001 + Solution Blueprint §3.4). | Sign-off obtained before /task generation. If absent at /task time, layout-specific Tasks will be deferred. |
| QS-006 | T-010 | supported-languages list (per `constitution/10-alm-configuration.md`) currently is en-US only. If multi-language is intended at first ship, confirm the language set before generating RESX scaffolding. | Multi-language-ready posture (Constitution §15) — generate RESX skeleton for en-US only at /implement; additional languages added later as a config event. |
| QS-007 | T-011 | supported-languages list (per `constitution/10-alm-configuration.md`) currently is en-US only. If multi-language is intended at first ship, confirm the language set before generating RESX scaffolding. | Multi-language-ready posture (Constitution §15) — generate RESX skeleton for en-US only at /implement; additional languages added later as a config event. |
| QS-008 | T-012 | Test environment readiness — does Test environment have upstream Fusion → D365 sync data populated to a representative volume? | Test data fixture (per Test Plan §6) is loaded; Fusion sync features 463/464/546/575 are live in Test. |
...
- `/tdd f465-account-management-informational-parent-hierarchy-in-d365` — should answer the schema-detail QUESTIONs.
- `/task f465-account-management-informational-parent-hierarchy-in-d365` — generates cards; QUESTIONs are embedded as caveats.
