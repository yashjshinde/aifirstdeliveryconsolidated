...
# Functional Design Document — Account Management - Bill‑To to Ship‑To Relationship

**Project:** IS - D365 Avanade — Service Operations on Dynamics 365 CE
**Based On:** Functional Specification v1.0 — see [spec.md](../../specs/f466-account-management-bill-to-to-ship-to-relationship/spec.md)
...
|---|---|---|
| Functional Specification | 1.0 | [spec.md](../../specs/f466-account-management-bill-to-to-ship-to-relationship/spec.md) |
| Spec Review | 1.1 | [review.md](../../specs/f466-account-management-bill-to-to-ship-to-relationship/review.md) |
...
This Functional Design Document is the authoritative reference for build, configuration, and User Acceptance Testing (UAT) of **Account Management - Bill‑To to Ship‑To Relationship** within the broader Epic 454 (E01-Customer Management). It is intended for developers, testers, and business stakeholders. The FDD elaborates the approved functional specification at [spec.md](../../specs/f466-account-management-bill-to-to-ship-to-relationship/spec.md) into structured design content; it does not introduce new requirements. Where the upstream specification was authored under `l3-intake: optional` (FRs deferred to `/plan`), this FDD carries that deferral forward and flags impacted sections in §16 Functional Gap Log.

### 2.2 Scope

This document covers the design for ALM Feature **466 — Account Management - Bill‑To to Ship‑To Relationship** under Epic 454 (E01-Customer Management). The capability is delivered as a single module:

- **Module 1:** Account Management - Bill‑To to Ship‑To Relationship — see §6.1.

Cross-references: see [spec.md](../../specs/f466-account-management-bill-to-to-ship-to-relationship/spec.md) §5 for the verbatim ALM description and acceptance criteria. FR-NNN range will be assigned by `/plan` from the L3 Product Backlog Item created at that step.
...
|---|---|---|
| L3 Product Backlog Item created in ADO under Feature 466 | Generated by `/plan` | FR-NNN can be authored |
...
**To-Be (Future State):** Account Management - Bill‑To to Ship‑To Relationship delivers the capability described in the spec to give service users the consolidated view / control they need within D365 CE, leveraging records synced from Fusion and any D365-native informational data added by this feature.
...
| CE-002 | `msdyn_customerasset` | Extension | Custom Column on existing entity | `msdyn_customerasset` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
| CE-003 | `msdyn_functionallocation` | Extension | Custom Column on existing entity | `msdyn_functionallocation` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
...
I want bill‑to and ship‑to relationships visible in D365

So that billing reflects customer preferences.

##### Original Acceptance Criteria (verbatim from ALM)

AC-1:
Given a bill‑to is linked to multiple ship‑tos in Fusion,
When synced with D365 ,
Then an equivalent relationship will be represented in D365 where one or more ship to (service account) is linked to a Bill to (billing account).

AC-2:
GIVEN a Bill to is also a Ship to in fusion
When synced with D365
Then an equivalent relationship will be represented in D365.  |
...
- The current data model provides this relationship: Account (Service account) has service address field which is the SHIP to address -> This record has a child function location record, which also has a service address. Both are set to same value during the Fusion to D365 sync. The functional location record may indicate the type as room, site, floor to make it easy to group the customer assets. (A service account represents a SHIP to equivalent of Fusion)
-  When an account is both Billing and Service account,

- The billing account will be a separate record - this will be created by the integration process
- The service account will be a separate record and its field "Billing account" will point to the billing account record.  @Baumberger, Lina (Avanade), @Bhatt, Aditi (Avanade) - Tweaked AC-2 to represent the data separation/split when Accounts are synced from Fusion to D365.
- The alt key on Account must be: Account Number + Site Number (SHIP to or Bill to in Fusion) + custom field to indicate Billing Account or Service Account.
...
| Table | `msdyn_customerasset` | Surface in 360 / form layout for this feature. | D365-native | D365-native |
