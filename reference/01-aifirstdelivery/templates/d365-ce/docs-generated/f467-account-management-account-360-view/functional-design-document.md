...
status: DRAFT
version: 1.0
spec-ref: specs/f467-account-management-account-360-view/spec.md
parent-epic-id: "454"
...
# Functional Design Document — Account Management - Account 360 View

**Project:** IS - D365 Avanade — Service Operations on Dynamics 365 CE
**Based On:** Functional Specification v1.0 — see [spec.md](../../specs/f467-account-management-account-360-view/spec.md)
...
|---|---|---|
| Functional Specification | 1.0 | [spec.md](../../specs/f467-account-management-account-360-view/spec.md) |
| Spec Review | 1.1 | [review.md](../../specs/f467-account-management-account-360-view/review.md) |
...
This Functional Design Document is the authoritative reference for build, configuration, and User Acceptance Testing (UAT) of **Account Management - Account 360 View** within the broader Epic 454 (E01-Customer Management). It is intended for developers, testers, and business stakeholders. The FDD elaborates the approved functional specification at [spec.md](../../specs/f467-account-management-account-360-view/spec.md) into structured design content; it does not introduce new requirements. Where the upstream specification was authored under `l3-intake: optional` (FRs deferred to `/plan`), this FDD carries that deferral forward and flags impacted sections in §16 Functional Gap Log.

### 2.2 Scope

This document covers the design for ALM Feature **467 — Account Management - Account 360 View** under Epic 454 (E01-Customer Management). The capability is delivered as a single module:

- **Module 1:** Account Management - Account 360 View — see §6.1.

Cross-references: see [spec.md](../../specs/f467-account-management-account-360-view/spec.md) §5 for the verbatim ALM description and acceptance criteria. FR-NNN range will be assigned by `/plan` from the L3 Product Backlog Item created at that step.
...
|---|---|---|
| L3 Product Backlog Item created in ADO under Feature 467 | Generated by `/plan` | FR-NNN can be authored |
...
**To-Be (Future State):** Account Management - Account 360 View delivers the capability described in the spec to give service users the consolidated view / control they need within D365 CE, leveraging records synced from Fusion and any D365-native informational data added by this feature.
...
| CE-002 | `msdyn_agreement` | Extension | Custom Column on existing entity | `msdyn_agreement` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
| CE-003 | `incident` | Extension | Custom Column on existing entity | `incident` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
| CE-004 | `contact` | Extension | Custom Column on existing entity | `contact` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
| CE-005 | `msdyn_functionallocation` | Extension | Custom Column on existing entity | `msdyn_functionallocation` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
| CE-006 | `invoice` | Extension | Custom Column on existing entity | `invoice` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
| CE-007 | `msdyn_workorder` | Extension | Custom Column on existing entity | `msdyn_workorder` | Simple | TBD (deferred to /plan) | Surface in 360 / form layout for this feature. |
...
### 6.1 Module: Account Management - Account 360 View

#### 6.1.1 Module Overview

This module covers the entirety of ALM Feature **467 — Account Management - Account 360 View**. It is a single-feature module in this FDD; sibling features under Epic 454 are scoped in their own per-feature FDDs. The module's primary personas are listed in §3.2; dependencies are documented in §3.4 and §14.2.
...
#### Feature: 467 — Account Management - Account 360 View
...
I want all the Associated records(Contacts, Agreements, Assets, Sites/Locations, Cases/Work Orders, Invoices etc.) tied to site‑level accounts:


So that Account 360 View is available.
...
Given an Account exists in D365,


When viewed in D365,


Then the Account form should show all the Associated records in respective grids/tabs as per the expected layout.


Associated records include but not limited to (Contacts, Agreements, Assets, Functional Location, Cases, Work Orders, Invoices etc.)
...
-
Functional team will create a simple wireframe in excel/miro/figma/visio collaborating with tech team for the Account form.

-
Tech team to customize the solution to include these changes

- Add/remove tabs/fields based on feedback from OPEX

- Remove unnecessary items from related record sitemap if applicable (Document this2
...
| Table | `msdyn_agreement` | Surface in 360 / form layout for this feature. | D365-native | D365-native |
