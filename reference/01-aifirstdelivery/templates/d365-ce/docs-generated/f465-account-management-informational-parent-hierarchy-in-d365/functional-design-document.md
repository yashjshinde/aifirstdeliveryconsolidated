...
status: DRAFT
version: 1.0
spec-ref: specs/f465-account-management-informational-parent-hierarchy-in-d365/spec.md
parent-epic-id: "454"
...
# Functional Design Document — Account Management - Informational Parent Hierarchy in D365

**Project:** IS - D365 Avanade — Service Operations on Dynamics 365 CE
**Based On:** Functional Specification v1.0 — see [spec.md](../../specs/f465-account-management-informational-parent-hierarchy-in-d365/spec.md)
...
|---|---|---|
| Functional Specification | 1.0 | [spec.md](../../specs/f465-account-management-informational-parent-hierarchy-in-d365/spec.md) |
| Spec Review | 1.1 | [review.md](../../specs/f465-account-management-informational-parent-hierarchy-in-d365/review.md) |
...
This Functional Design Document is the authoritative reference for build, configuration, and User Acceptance Testing (UAT) of **Account Management - Informational Parent Hierarchy in D365** within the broader Epic 454 (E01-Customer Management). It is intended for developers, testers, and business stakeholders. The FDD elaborates the approved functional specification at [spec.md](../../specs/f465-account-management-informational-parent-hierarchy-in-d365/spec.md) into structured design content; it does not introduce new requirements. Where the upstream specification was authored under `l3-intake: optional` (FRs deferred to `/plan`), this FDD carries that deferral forward and flags impacted sections in §16 Functional Gap Log.

### 2.2 Scope

This document covers the design for ALM Feature **465 — Account Management - Informational Parent Hierarchy in D365** under Epic 454 (E01-Customer Management). The capability is delivered as a single module:

- **Module 1:** Account Management - Informational Parent Hierarchy in D365 — see §6.1.

Cross-references: see [spec.md](../../specs/f465-account-management-informational-parent-hierarchy-in-d365/spec.md) §5 for the verbatim ALM description and acceptance criteria. FR-NNN range will be assigned by `/plan` from the L3 Product Backlog Item created at that step.
...
|---|---|---|
| L3 Product Backlog Item created in ADO under Feature 465 | Generated by `/plan` | FR-NNN can be authored |
...
**To-Be (Future State):** Account Management - Informational Parent Hierarchy in D365 delivers the capability described in the spec to give service users the consolidated view / control they need within D365 CE, leveraging records synced from Fusion and any D365-native informational data added by this feature.
...
#### Feature: 465 — Account Management - Informational Parent Hierarchy in D365
...
I want an informational parent grouping in D365

So that I can understand enterprise relationships without impacting Fusion.

##### Original Acceptance Criteria (verbatim from ALM)

AC-1:
Given multiple service accounts belong to one organization,
When viewed in D365,
Then hierarchy is in place on Accounts for the user to be able to add a relation between two accounts.

AC-2: Given the requirement is implemented,
When the user adds a parent Account
Then no parent hierarchy data is sent back to Fusion. |
...
- The parent account field on the account record will be used to set a parent account for service and billing accounts. There is no integration or migration for this field. This is newly introduced in D365 to group all related accounts and will be manually created in D365.
- This does not exist today in Fusion, so the two tasks associated with this story are:

- A parent account record must be created - this is not a service account OR billing account just a record to connect all related accounts, the required fields will be set manually though they will not be used in any existing business logic.
- The parent field on the service and billing account records must be set to this manually in D365
- To achieve both a & b

- Create a new security role called Operational Admin (note, this is also mentioned in one another work item) as a copy of one of OOB least privilege security role (basic role?)
-
Assign the
privileges - organization level, can create account, update account, read account, append and append to.


- As there is no integration back from D365 to Fusion AC-2 is satisfied.
