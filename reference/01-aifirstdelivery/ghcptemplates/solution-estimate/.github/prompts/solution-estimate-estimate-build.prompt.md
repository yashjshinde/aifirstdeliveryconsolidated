---
mode: agent
description: "Generate the formal 3-part estimation deliverable — Business Req Detail, Module Build Hours, Module Overall Hours. Triggers on: 'estimate-build', 'formal estimate', 'estimation deliverable'."
---
...
```
/solution-estimate-estimate-build {project}
```
...
1. Check that at least one estimate file exists in `estimates/{project}/`:
   ...
   If none found: "No estimates found for project '{project}'. Run `/solution-estimate-estimate-rom`, `/solution-estimate-estimate-spec`, or `/solution-estimate-estimate-plan` first."
...
2. Check for proposed factors:
   ...
   - If not FACTORS APPROVED or file missing: stop — "New factors are pending review. Run `/solution-estimate-factors-review {project}` first."
