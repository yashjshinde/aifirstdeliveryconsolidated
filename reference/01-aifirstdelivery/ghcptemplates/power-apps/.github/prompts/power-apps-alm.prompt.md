---
mode: agent
description: "Manage Azure DevOps work item synchronisation for Power Platform. Triggers on: 'alm', 'sync work items'."
---
...
```
/power-apps-alm extract {feature-name}
/power-apps-alm sync    {feature-name} {uid} {alm-id}
/power-apps-alm sync    {feature-name} --file {path-to-alm-response.json}
/power-apps-alm get     {feature-name} {alm-id}
```

Read the first argument after `/power-apps-alm` to determine the sub-command, then follow the matching section.
...
5. Write to `output/{feature-name}/power-apps-alm/extract-{YYYYMMDD-HHmmss}.json`.
6. Print: item counts by level, output file path, and the instruction:
   ...
   Output   : output/{feature}/power-apps-alm/extract-{timestamp}.json
   Next step: Pass this file to the ALM Agent to create work items.
              The ALM Agent must call  /power-apps-alm sync {feature-name} --file {response.json}
...
**Single item (inline):**
```
/power-apps-alm sync account-loyalty-points account-loyalty-points-T-001 12345
```

**Bulk file** (sent by the ALM Agent after creating all work items):
```
/power-apps-alm sync account-loyalty-points --file output/account-loyalty-points/alm/alm-response.json
```
...
```
/power-apps-alm sync-testplan {feature-name}
/power-apps-alm sync-testplan {feature-name} {TC-ID} {ALM-ID}
```
...
Example: `/power-apps-alm sync-testplan my-feature TC-CA001 12345`
...
Not found (in mapping but not in files):
  {TC-ID}  — check TC ID spelling or re-run /power-apps-extract testplan to verify
...
```
/power-apps-alm get account-loyalty-points 10004
```
...
2. Find the item where `alm-id = {alm-id}`. If not found, stop:
   ...
   Run /power-apps-alm sync first to ensure ALM IDs have been written.
   ```
...
5. Write to `output/{feature-name}/power-apps-alm/get-{alm-id}-{YYYYMMDD}.json`.
6. Print:
...
Output    : output/{feature}/power-apps-alm/get-{alm-id}-{date}.json
```
