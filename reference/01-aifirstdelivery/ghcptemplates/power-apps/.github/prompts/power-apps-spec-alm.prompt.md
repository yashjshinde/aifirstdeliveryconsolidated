---
mode: agent
description: "Generate a Power Platform spec from structured ALM work items. Triggers on: 'spec-alm', 'structured intake'."
---

Enhance structured ALM work items (L1/L2/L3) from your ALM tool into a Power Platform functional specification.
...
```
/power-apps-spec-alm {feature-name}
```
...
2. Check `requirement-intake`. If it is not `structured`, stop:
   ...
   Use /power-apps-spec for plain-language requirements.
    ...
    - If `required`: every L2 in the input must have at least one L3 item. After parsing (step 5), stop if any L2 has no L3:
      ...
      Options:
        ...
        2. Set l3-intake: optional in constitution/10-alm-configuration.md
           to allow /power-apps-plan to generate missing L3 stories.
      ```
    - If `optional`: L3 may be absent or partially provided — continue. `/power-apps-plan` will generate L3 stories for any L2 with no coverage.
...
5. Parse the provided work items:
   ...
   - Preserve ALM IDs exactly as provided — they will be referenced by `/power-apps-plan`
   - **If `l3-intake: optional`:** For any L2 with no L3 items in the input, record it as a pending L2: set `source: pending` in the `alm-ids` YAML front matter and write `pending` as the Source column value in the Traceability Matrix. Do not stop — `/power-apps-plan` will decompose it into L3 stories.
...
7. For each L3 item, enhance the description:
   ...
   - **Acceptance Criteria:** If the ALM item provides acceptance criteria, preserve them verbatim first, then add further Given/When/Then scenarios (minimum total 2: one positive, one negative). If no AC is provided in the ALM item, generate from the story description.
   - Identify Business Rules (BR-NNN) — delegation-safe pattern notes, validations, conditions
   - Identify Power Platform Impact: Canvas App screens, MDA forms/views, flows, Copilot topics, Dataverse tables affected
   - Flag delegation risks if large data volumes are mentioned
   - Flag Non-Functional Considerations: performance, security (row/column-level), scalability, reliability
   ...
   **If `l3-intake: optional` and a L2 has no L3 items (pending):**
   - Do NOT generate FR-NNN from the L2 description — FRs for this branch will be produced by `/power-apps-plan`
   - Write a pending placeholder block in §5 under the L2 heading:
     ...
     **Status:** No L3 items were provided for this L2 branch.
     `/power-apps-plan` will decompose {L2-ALM-ID} into {L3-Type} stories and generate Tasks.
     ```
...
8. Generate the spec using `specs/_template-structured.md` as the authoritative structure.
   ...
   - In §14 ALM Traceability Matrix, add a `Source` column — `alm` for provided L3s, `pending` for gaps
...
10. Print summary:
    ...
    {L1-Type}   : {n}  (IDs: {list})
    {L2-Type}   : {n}  (IDs: {list})
    {L3-Type}   : {n} from ALM{, {n} pending — to be created by /power-apps-plan}
    FR-NNN      : {n} requirements generated
    BR-NNN      : {n} business rules identified
    Open Qs     : {n}
    Output      : specs/{feature-name}/spec.md
    
    Next step: /power-apps-review {feature-name}
    ```
...
- Every BR-NNN referenced inline must appear in the consolidated §7 Business Rules table
- Every ALM-provided L3 (`source: alm`) must map to at least one FR-NNN in §14 ALM Traceability Matrix — pending placeholders (`source: pending`) are exempt; FR-NNN will be generated by `/power-apps-plan`
...
| Acceptance Criteria | ALM-provided AC preserved verbatim first; generated Given/When/Then to reach minimum 2 (positive + negative) per L3 |
| Business Rules | Delegation-safe notes, validations, conditions implied by the story |
| Power Platform Impact | Canvas screens, MDA forms, flows, Copilot topics, Dataverse tables |
| Data Considerations | Delegation risks, data volumes, sensitive field identification |
| Non-Functional | Performance, security, scalability, reliability (Power Platform specific) |
...
- **If `l3-intake: optional`:** Do NOT create new L3 IDs in `/power-apps-spec-alm` — mark any missing L3 as pending: set `source: pending` in the YAML `alm-ids` front matter and `pending` in the Traceability Matrix Source column; `/power-apps-plan` will generate them
- Do NOT reorganise the hierarchy — the L1/L2/L3 structure from the ALM tool is the authority
- Spec is functional only — no Power Fx formulas, no flow definitions, no technical decisions
- Flag `intake: structured` in the YAML front matter — `/power-apps-plan` reads this to determine mode
- Flag `l3-intake: required` or `l3-intake: optional` in the YAML front matter — `/power-apps-plan` reads this to know whether to generate L3 stories for pending branches
- Always flag delegation risks for any gallery or filter over large datasets
