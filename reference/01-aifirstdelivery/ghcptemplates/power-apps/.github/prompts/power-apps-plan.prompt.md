---
mode: agent
description: "Generate a technical plan and work-items.yaml for a Power Platform feature from an approved functional specification. Triggers on: 'plan', 'generate plan', 'technical plan'."
---
...
1b. Read `constitution/10-alm-configuration.md` — check `brownfield.enabled`.
    ...
    - If absent or not IMPACT-ASSESSED, stop: "Brownfield mode requires an impact analysis. Run `/power-apps-impact {feature-name}` first."
...
`/power-apps-plan` must NOT create new L1/L2/L3 items — it generates **Task-level (L4) items only**,
...
in the §14 ALM Traceability Matrix). `/power-apps-plan` identifies pending branches by reading the Source column
...
- id: "{feature-name}-{L3-Prefix}-001"
  ...
  source: generated          # created by /power-apps-plan — ALM Agent will push this to ADO
...
- id: "{feature-name}-T-NNN"
  ...
  alm-type: Task
  title: "..."
  source: generated-parent   # parent L3 was generated by /power-apps-plan, not from ALM
...
> **Source:** Generated by `/power-apps-plan` — no L3 items were provided for parent {L2-ALM-ID}.
> The ALM Agent (`/wi-create-bulk`) will create this story in the ALM tool.

**Mapped FRs:** {FR-NNN if derivable from L2 scope, otherwise "— (to be assigned after /power-apps-review)"}
