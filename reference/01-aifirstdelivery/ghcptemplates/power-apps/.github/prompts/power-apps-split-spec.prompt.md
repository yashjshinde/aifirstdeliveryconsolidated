---
mode: agent
description: "Split a mixed functional specification into domain-scoped specs for Power Apps, Integration, Data Migration, and Reporting agents. Triggers on: 'split-spec', 'split spec', 'split specification'."
---
...
```
/power-apps-split-spec {feature-name}
```
...
1. Read `specs/{feature-name}/spec.md`. If the file does not exist, stop: "No spec found. Run /power-apps-spec first."
...
version: 1.0
date: {YYYY-MM-DD}
author: Claude Code (/power-apps-split-spec)
---
```
...
split-by: Claude Code (/power-apps-split-spec)
---
...
- [ ] Power Apps agent: run `/power-apps-review {feature-name}` to validate the Power Apps spec
- [ ] Integration agent: copy `specs/{feature-name}-integration/` and run `/power-apps-review {feature-name}-integration`
- [ ] Data Migration agent: copy `specs/{feature-name}-data-migration/` and run `/power-apps-review {feature-name}-data-migration`
- [ ] Reporting agent: copy `specs/{feature-name}-reporting/` and run `/power-apps-review {feature-name}-reporting`
- [ ] Ensure cross-cutting FR dependencies are reflected in all plans when `/power-apps-plan` is run
```
...
Power Apps Next Step     : /power-apps-review {feature-name}
Integration Next Step    : /power-apps-review {feature-name}-integration      ← run in the Integration agent
Data Migration Next Step : /power-apps-review {feature-name}-data-migration   ← run in the Data Migration agent
Reporting Next Step      : /power-apps-review {feature-name}-reporting        ← run in the Reporting agent
```
