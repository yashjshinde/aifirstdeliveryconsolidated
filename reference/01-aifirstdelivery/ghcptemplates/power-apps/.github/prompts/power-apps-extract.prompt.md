---
mode: agent
description: "Extract Power Platform test plan, suites, and cases to ALM-ready formats. Triggers on: 'extract', 'extract testplan'."
---
...
```
/power-apps-extract testplan  {feature-name}
/power-apps-extract testsuites {feature-name} [suite-name]
/power-apps-extract testcases  {feature-name} [tc-id]
```

Read the first argument after `/power-apps-extract` to determine the sub-command, then follow the matching section.
...
7. Print the completion report:
   ...
   Next step: /power-apps-alm sync-testplan {f}  (after creating test cases in ALM)
   ```
