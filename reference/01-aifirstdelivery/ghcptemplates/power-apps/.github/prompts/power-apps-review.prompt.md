---
mode: agent
description: "Review a Power Platform spec for completeness and constitution compliance. Triggers on: 'review', 'spec review'."
---
...
```
/power-apps-review {feature-name}
```
...
3. **Multi-Domain Detection — run first, before any other check.**
   ...
   - **BLOCKER text (Integration):** "This spec contains Azure Integration requirements that are out of scope for the Power Apps agent. Run `/power-apps-split-spec {feature-name}` to separate the Power Apps and integration portions before proceeding. The integration spec must then be handed to the Azure Integration agent."
   - **BLOCKER text (Data Migration):** "This spec contains Data Migration requirements (ADF pipelines, SFTP, staging) that are out of scope for the Power Apps agent. Run `/power-apps-split-spec {feature-name}` to separate the Power Apps and data migration portions before proceeding. The data migration spec must then be handed to the Data Migration agent."
...
3c. **Cross-Feature Spec Scan** — informational only; does not block approval unless a hard CONFLICT is identified:
    ...
    - If a CONFLICT is detected (two features claiming incompatible modifications to the same component): raise as RECOMMENDED — "Component {name} is also claimed by feature {other-feature} with action {action}. Resolve the overlap before /power-apps-plan to avoid deployment conflicts."
...
|---|---|---|
| BLOCKER | Constitution violation or out-of-domain requirement | Sets NEEDS REWORK; blocks `/power-apps-plan` |
| REQUIRED | Missing information that prevents technical planning | Sets NEEDS REWORK; blocks `/power-apps-plan` |
...
- `APPROVED` — zero BLOCKERs, zero REQUIREDs. `/power-apps-fdd`, `/power-apps-testplan`, and `/power-apps-plan` may proceed.
- `NEEDS REWORK` — one or more BLOCKERs or REQUIREDs. `/power-apps-plan` refuses to proceed.
- `/power-apps-fdd` may be started in parallel with review if the spec is substantially complete, but must not be marked final until review is APPROVED.
...
Next step    : /power-apps-fdd {feature-name}

{If NEEDS REWORK:}
Output       : specs/{feature-name}/review.md
Resolve all BLOCKERs and REQUIREDs, then re-run /power-apps-review {feature-name}.
