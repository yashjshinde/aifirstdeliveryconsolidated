---
mode: agent
description: "Split a mixed-domain reporting spec into domain-scoped specs. Triggers on: 'split-spec', 'mixed domain'."
---
...
```
/reporting-split-spec {feature-name}
```
...
1. Read `specs/{feature-name}/spec.md`. If the file does not exist, stop: "No spec found. Run /reporting-spec first."
...
version: 1.0
date: {YYYY-MM-DD}
author: Claude Code (/reporting-split-spec)
---
```
...
split-by: Claude Code (/reporting-split-spec)
---
...
- [ ] Reporting agent: run `/reporting-review {feature-name}` to validate the Reporting spec
- [ ] Integration agent: copy `specs/{feature-name}-integration/` and run `/reporting-review {feature-name}-integration`
- [ ] CE agent: copy `specs/{feature-name}-ce/` and run `/reporting-review {feature-name}-ce`
- [ ] Data Migration agent: copy `specs/{feature-name}-data-migration/` and run `/reporting-review {feature-name}-data-migration`
- [ ] Ensure cross-cutting FR dependencies are reflected in all plans when `/reporting-plan` is run
```
...
Reporting Next Step      : /reporting-review {feature-name}
Integration Next Step    : /reporting-review {feature-name}-integration   ← run in the Integration agent
CE Next Step             : /reporting-review {feature-name}-ce            ← run in the D365 CE agent
Data Migration Next Step : /reporting-review {feature-name}-data-migration ← run in the Data Migration agent
```
