---
mode: agent
description: "Extract Reporting test plan, suites, and cases to ALM-ready formats. Triggers on: 'extract', 'extract testplan'."
---
...
```
/reporting-extract testplan  {feature-name}
/reporting-extract testsuites {feature-name} [suite-name]
/reporting-extract testcases  {feature-name} [tc-id]
```

Read the first argument after `/reporting-extract` to determine the sub-command, then follow the matching section.
...
7. Print the completion report:
   ...
   Next step: /reporting-alm sync-testplan {f}  (after creating test cases in ALM)
   ```
