---
mode: agent
description: "Manage Azure DevOps work item synchronisation for Reporting. Triggers on: 'alm', 'sync work items'."
---

Manage ALM work item synchronisation for a Reporting feature.
Reads plans/{feature}/work-items.yaml and constitution/10-alm-configuration.md.

## Usage

```
/reporting-alm extract {feature-name}
/reporting-alm sync    {feature-name} {uid} {alm-id}
/reporting-alm sync    {feature-name} --file {path-to-alm-response.json}
/reporting-alm get     {feature-name} {alm-id}
```

Read the first argument after `/reporting-alm` to determine the sub-command, then follow the matching section.
...
5. Write to `output/{feature-name}/reporting-alm/extract-{YYYYMMDD-HHmmss}.json`.
6. Print: item counts by level, output file path, and the instruction:
   ...
   Output   : output/{feature}/reporting-alm/extract-{timestamp}.json
   Next step: Pass this file to the ALM Agent to create work items.
              The ALM Agent must call  /reporting-alm sync {feature-name} --file {response.json}
...
```json
{
  ...
  "work-items": [
    ...
    },
    {
      ...
      "component-type": "{Dataset | Measure | Report | RLS | SSRS | Workspace | StoredProc}",
      "complexity": "M",
      "priority": "High",
      "priority-mapped": 1,
      "status": "TODO",
...
- **Rich content:** All `description` and `acceptance-criteria` values must contain the full markdown text verbatim from the source YAML — do NOT flatten multi-line content or strip markdown syntax. The ALM Agent converts markdown to the target tool's rich text format.
...
**Single item (inline):**
```
/reporting-alm sync sales-dashboard sales-dashboard-T-001 12345
```

**Bulk file** (sent by the ALM Agent after creating all work items):
```
/reporting-alm sync sales-dashboard --file output/sales-dashboard/alm/alm-response.json
```
...
3. For each uid → alm-id pair:
   ...
   b. Update plan.md:
      - For Level 1 / 2 / 3 items: find the heading line containing the item ID and replace `**ALM ID:** *(pending)*` with `**ALM ID:** {alm-id}`.
      - For Level 4 items: find the task row in the `##### Tasks` table and replace `*(pending)*` in the ALM ID column with `{alm-id}`. Also update the matching row in the `# 2. Task Inventory` table.
   c. If the item is Level 4: also find the matching task card in `tasks/{feature-name}/` by `task-id` and set `alm-id: {alm-id}` in the task card front matter.
4. Update `specs/{feature-name}/spec.md` Section 13 Traceability Matrix with ALM Work Item IDs:
   ...
   - If the traceability matrix header does not contain an `ALM Work Item IDs` column, add it as the last column.
   - For each FR data row: populate the ALM Work Item IDs cell with comma-separated `#{alm-id}` values. Use `—` if no work items reference this FR.
...
```
/reporting-alm sync-testplan {feature-name}
/reporting-alm sync-testplan {feature-name} {TC-ID} {ALM-ID}
```
...
Example: `/reporting-alm sync-testplan sales-dashboard TC-SALES-DA-001 12345`
...
**B. Update the suite file** — use the TC ID pattern to identify the target file:
| Pattern | File |
|---|---|
| …DA… | `docs-generated/{feature-name}/test-cases/data-accuracy.md` |
| …UAT… | `docs-generated/{feature-name}/test-cases/uat.md` |
| …SEC… or …RLS… | `docs-generated/{feature-name}/test-cases/rls-security.md` |
| …R… (regression) | `docs-generated/{feature-name}/test-cases/regression.md` |
...
Updated successfully:
  TC-SALES-DA-001  →  12345   test-plan ✓   data-accuracy.md ✓
  TC-SALES-UAT-001 →  12348   test-plan ✓   uat.md ✓

Not found (TC ID in mapping but not in files):
  {TC-ID}  — check TC ID spelling or re-run /reporting-extract testplan to verify
...
**Purpose:** Return the current full state of a specific work item by its ALM ID.

### Steps

```
/reporting-alm get sales-dashboard 10004
```
...
2. Find the item where `alm-id = {alm-id}`. If not found, stop:
   ...
   Run /reporting-alm sync first to ensure ALM IDs have been written.
   ```
3. If the item is Level 4: read the matching task card from `tasks/{feature-name}/` to get the current `status`, `validation-status`, and `impl-doc-path`.
...
5. Write to `output/{feature-name}/reporting-alm/get-{alm-id}-{YYYYMMDD}.json`.
6. Print:
...
Output    : output/{feature}/reporting-alm/get-{alm-id}-{date}.json
```
