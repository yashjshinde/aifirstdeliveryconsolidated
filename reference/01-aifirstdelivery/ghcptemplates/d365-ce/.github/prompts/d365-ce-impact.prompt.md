---
mode: agent
description: "Brownfield impact analysis for a D365 CE feature. Classifies every component as NEW/EXTEND/REPLACE/REFERENCED/CONFLICT. Triggers on: 'impact', 'brownfield impact', 'impact analysis'."
---

# /d365-ce-impact — Brownfield Impact Analysis
...
Required by `/d365-ce-plan` when `brownfield.enabled: true`.

## Usage

```
/d365-ce-impact {feature-name}
```
...
2. Read `specs/{feature-name}/review.md`.
   If status is not `APPROVED`, stop: "Spec is not approved. Run `/d365-ce-review {feature-name}` first."
...
5. Read `specs/{feature-name}/spec.md` — focus on:
   ...
   - §14 Brownfield Context if already populated by `/d365-ce-spec`
...
Next step : /d365-ce-plan {feature-name}
```
