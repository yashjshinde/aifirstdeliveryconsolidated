---
mode: agent
description: "Implement a D365 CE task card — generate code, tests, and implementation record. Use when the user wants to generate code for a validated task card. Triggers on: 'implement', 'generate code', 'implement task', 'write code for task'."
---
...
3. If `validation-status` is not `READY TO IMPLEMENT`, stop immediately:
   - If `NEEDS REWORK`: "This task has not passed validation. Run `/d365-ce-validate {feature-name}` first and fix all reported issues."
   ...
   - If field is missing: "This task has not been validated. Run `/d365-ce-validate {feature-name}` before implementing."
