---
mode: agent
description: "Generate a D365 CE technical plan from an approved spec. Use when the user wants to break down an approved functional spec into work items, plan.md, and work-items.yaml. Triggers on: 'plan', 'create plan', 'technical plan', 'break down spec'."
---
...
2. If status is not `APPROVED`, stop: "This spec has not been approved. Run `/d365-ce-review` first and resolve all BLOCKERs and REQUIREDs."

2b. **Brownfield Check** — check `brownfield.enabled` in `constitution/10-alm-configuration.md`.
    If `true`:
    - Check `specs/{feature-name}/impact-analysis.md` exists. If not, stop: "Brownfield mode is enabled. Run `/d365-ce-impact {feature-name}` before planning."
...
`/d365-ce-plan` must NOT create new L1/L2/L3 items — it generates **Task-level (L4) items only**,
...
in the §13 ALM Traceability Matrix). `/d365-ce-plan` identifies pending branches by reading the Source column
...
- id: "{feature-name}-{L3-Prefix}-001"
  ...
  source: generated          # created by /d365-ce-plan — ALM Agent will push this to ADO
...
- id: "{feature-name}-T-NNN"
  ...
  alm-type: Task
  title: "..."
  source: generated-parent   # parent L3 was generated by /d365-ce-plan, not from ALM
...
> **Source:** Generated by `/d365-ce-plan` — no L3 items were provided for parent {L2-ALM-ID}.
> The ALM Agent (`/wi-create-bulk`) will create this story in the ALM tool.

**Mapped FRs:** {FR-NNN if derivable from L2 scope, otherwise "— (to be assigned after /d365-ce-review)"}
