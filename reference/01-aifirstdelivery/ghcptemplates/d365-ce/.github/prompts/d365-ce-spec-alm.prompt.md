---
mode: agent
description: "Enhance structured ALM work items (L1/L2/L3) into a D365 CE functional spec. Use when requirements already exist as work items in ADO/Jira. Triggers on: 'spec-alm', 'structured intake', 'import work items'."
---

Enhance structured ALM work items (L1/L2/L3) from your ALM tool into a D365 CE functional specification.
...
2. Check `requirement-intake`. If it is not `structured`, stop:
   ...
   Use /d365-ce-spec for plain-language requirements.
    ...
    - If `required`: every L2 in the input must have at least one L3 item. After parsing (step 5), stop if any L2 has no L3:
      ...
      Options:
        ...
        2. Set l3-intake: optional in constitution/10-alm-configuration.md
           to allow /d365-ce-plan to generate missing L3 stories.
      ```
    - If `optional`: L3 may be absent or partially provided — continue. `/d365-ce-plan` will generate L3 stories for any L2 with no coverage.
...
5. Parse the provided work items:
   ...
   - Preserve ALM IDs exactly as provided — they will be referenced by `/d365-ce-plan`
   - **If `l3-intake: optional`:** For any L2 with no L3 items in the input, record it as a pending L2: set `source: pending` in the `alm-ids` YAML front matter and write `pending` as the Source column value in the Traceability Matrix. Do not stop — `/d365-ce-plan` will decompose it into L3 stories.
...
7. For each L3 item, enhance the description:
   ...
   - Identify D365 CE Impact: entities, forms, views, plugins, flows, PCF controls affected
   - Flag Non-Functional Considerations from the story context
   ...
   **If `l3-intake: optional` and a L2 has no L3 items (pending):**
   - Do NOT generate FR-NNN from the L2 description — FRs for this branch will be produced by `/d365-ce-plan`
   - Write a pending placeholder block in §5 under the L2 heading:
     ...
     **Status:** No L3 items were provided for this L2 branch.
     `/d365-ce-plan` will decompose {L2-ALM-ID} into {L3-Type} stories and generate Tasks.
     ```
...
10. Print summary:
    ...
    {L1-Type}   : {n}  (IDs: {list})
    {L2-Type}   : {n}  (IDs: {list})
    {L3-Type}   : {n} from ALM{, {n} pending — to be created by /d365-ce-plan}
    FR-NNN      : {n} requirements generated
    BR-NNN      : {n} business rules identified
    Open Qs     : {n}
    Output      : specs/{feature-name}/spec.md
    
    Next step: /d365-ce-review {feature-name}
    ```
...
- Every ALM-provided L3 (`source: alm`) must map to at least one FR-NNN in §13 ALM Traceability Matrix — pending placeholders (`source: pending`) are exempt; FR-NNN will be generated by `/d365-ce-plan`
...
| Business Rules | Any rule conditions, validations, constraints implied by the story |
| D365 CE Impact | Entities, forms, fields, plugins, flows affected |
| Non-Functional | Performance, security, scalability, reliability specifics |
...
- **If `l3-intake: optional`:** Do NOT create new L3 IDs in `/d365-ce-spec-alm` — mark any missing L3 as pending: set `source: pending` in the YAML `alm-ids` front matter and `pending` in the Traceability Matrix Source column; `/d365-ce-plan` will generate them
- Do NOT reorganise the hierarchy — the L1/L2/L3 structure from the ALM tool is the authority
- Spec is functional only — no technical decisions, no code, no implementation detail
- Flag `intake: structured` in the YAML front matter — `/d365-ce-plan` reads this to determine mode
- Flag `l3-intake: required` or `l3-intake: optional` in the YAML front matter — `/d365-ce-plan` reads this to know whether to generate L3 stories for pending branches
