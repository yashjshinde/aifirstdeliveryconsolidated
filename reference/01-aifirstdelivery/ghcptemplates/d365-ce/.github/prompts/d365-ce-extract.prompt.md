---
mode: agent
description: "Extract test plan, suites, or test cases into ALM-ready CSV and JSON files for a D365 CE feature. Use when the user wants to export test cases to an ALM tool like Azure DevOps. Triggers on: 'extract test plan', 'export test cases', 'ALM test import', 'test case extract'."
---

Extract test plan, suites, or individual test cases for a D365 CE feature into ALM-ready output files. Rich content (markdown, steps as structured arrays) is preserved intact in the JSON output — the primary ALM import artifact.

## Usage

```
/d365-ce-extract testplan  {feature-name}
/d365-ce-extract testsuites {feature-name} [suite-name]
/d365-ce-extract testcases  {feature-name} [tc-id]
```

Read the first argument after `/d365-ce-extract` to determine the sub-command, then follow the matching section.
...
1. Read `docs-generated/{feature-name}/test-plan-and-strategy.md`.
   ...
   - Parse each suite reference table (§3.1–§3.4): TC ID, ALM ID, Title, Priority, Mapped FR, Context.
   - Note the file link for each suite.

2. Read all four suite files and extract full content with rich formatting intact:
   - `docs-generated/{feature-name}/test-cases/system-integration.md`
   ...
   **Primary ALM import artifact — full rich content preserved.** Structure:
   ```json
   {
     ...
     "test-suites": [
       {
         "suite-id": "TS-001",
         "suite-name": "System / Integration",
         "suite-file": "docs-generated/{feature-name}/test-cases/system-integration.md",
         "tc-count": 0,
         "test-cases": [
           {
             "tc-id": "TC-CRM-SI-001",
...
7. Print the completion report:
   ...
   Feature      : {feature-name}
   Suites       : 4
     TS-001  System / Integration : {N} TCs, {N} steps
     TS-002  UAT                  : {N} TCs, {N} steps
     TS-003  Security             : {N} TCs, {N} steps
   ...
   Next step: /d365-ce-alm sync-testplan {f}  (after creating test cases in ALM)
   ```
...
**suite-name options:** `system-integration` | `uat` | `security` | `regression` | `all` (default: `all`)
...
2. For each suite to extract, read the suite file:
   | Argument | File |
   |---|---|
   | `system-integration` | `docs-generated/{feature-name}/test-cases/system-integration.md` |
...
**tc-id:** A single TC ID (e.g., `TC-CRM-SI-001`) or omit to extract every test case from all suites.

### TC ID routing

| Prefix | Suite file |
|---|---|
| TC-CRM-SI… | `system-integration.md` |
| TC-CRM-UAT… | `uat.md` |
| TC-CRM-SEC… | `security.md` |
| TC-CRM-R… | `regression.md` |

### Steps

1. Determine scope:
   - If `tc-id` is provided: use the prefix to identify the likely suite file, then confirm. If not found in the expected file, scan all suite files.
...
9. Print the completion report:
   ...
   Filter       : {tc-id | all}
   Test Cases   : {N}
     TC-CRM-SI-001  System/Integration  3 steps  ALM: 12345
     TC-CRM-UAT-001  UAT               4 steps  ALM: —
     ...
   Total Steps  : {N}
