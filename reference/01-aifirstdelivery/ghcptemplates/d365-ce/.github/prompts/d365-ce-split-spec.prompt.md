---
mode: agent
description: "Split a mixed D365 CE + Integration / Data Migration / Reporting spec into domain-scoped specs. Triggers on: 'split-spec', 'split spec', 'split mixed spec'."
---
...
```
/d365-ce-split-spec {feature-name}
```
...
1. Read `specs/{feature-name}/spec.md`. If the file does not exist, stop: "No spec found. Run /d365-ce-spec first."
...
version: 1.0
date: {YYYY-MM-DD}
author: Claude Code (/d365-ce-split-spec)
---
```
...
split-by: Claude Code (/d365-ce-split-spec)
---
...
- [ ] CE agent: run `/d365-ce-review {feature-name}` to validate the CE spec
- [ ] Integration agent: copy `specs/{feature-name}-integration/` and run `/d365-ce-review {feature-name}-integration`
- [ ] Data Migration agent: copy `specs/{feature-name}-data-migration/` and run `/d365-ce-review {feature-name}-data-migration`
- [ ] Reporting agent: copy `specs/{feature-name}-reporting/` and run `/d365-ce-review {feature-name}-reporting`
- [ ] Ensure cross-cutting FR dependencies are reflected in all plans when `/d365-ce-plan` is run
```
...
CE Next Step             : /d365-ce-review {feature-name}
Integration Next Step    : /d365-ce-review {feature-name}-integration      ← run in the Integration agent
Data Migration Next Step : /d365-ce-review {feature-name}-data-migration   ← run in the Data Migration agent
Reporting Next Step      : /d365-ce-review {feature-name}-reporting        ← run in the Reporting agent
```
...
- CE spec retains the original file path — downstream commands (`/d365-ce-review`, `/d365-ce-plan`) do not need to change paths.
