...
Spec-driven development template for Dynamics 365 Customer Engagement using Claude Code.
...
/review     → specs/{feature}/review.md        ← APPROVED gate
     ↓
[Mixed domain? Integration or data migration requirements detected in spec]
/split-spec → specs/{feature}/spec.md (CE-scoped)
             + specs/{feature}-integration/spec.md (Integration agent — if integration FRs exist)
...
/validate   → updates validation-status on each task card  ← READY TO IMPLEMENT gate
     ↓
/implement  → output/{feature}/src/             (generated code + tests)
/document   → docs-generated/{feature}/         (plugin-registration, deployment-guide, release-notes)
```
...
| `/split-spec` | review APPROVED + mixed domain detected | `specs/{f}/spec.md` (CE-scoped) + `specs/{f}-integration/spec.md` (if INT FRs) + `specs/{f}-data-migration/spec.md` (if DM FRs) + `specs/{f}/split-manifest.md` |
...
| `/implement` | validation-status = READY TO IMPLEMENT | `output/{f}/src/` |
