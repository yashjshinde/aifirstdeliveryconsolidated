---
mode: agent
description: "Assign all work items for a domain feature to an Azure DevOps iteration (sprint). Triggers on: 'assign to sprint', 'sprint assign', 'sprint-assign'."
---
...
4. Read `{domain-path}/plans/{feature}/work-items.yaml`. Collect all `alm-id` values that are set.
   - If none: stop with "No ALM IDs found. Run /alm-wi-create-bulk first."
