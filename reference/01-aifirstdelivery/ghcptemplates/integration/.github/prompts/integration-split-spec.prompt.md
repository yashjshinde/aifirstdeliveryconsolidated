---
mode: agent
description: "Split a mixed-domain integration spec into domain-scoped specs. Triggers on: 'split-spec', 'mixed domain'."
---
...
```
/integration-split-spec {feature-name}
```
...
1. Read `specs/{feature-name}/spec.md`. If the file does not exist, stop: "No spec found. Run /integration-spec first."
...
version: 1.0
date: {YYYY-MM-DD}
author: Claude Code (/integration-split-spec)
---
```
...
split-by: Claude Code (/integration-split-spec)
---
...
- [ ] Integration agent: run `/integration-review {feature-name}` to validate the integration spec
- [ ] CE/PA/F&O agent: copy `specs/{feature-name}-ce/` and run `/integration-review {feature-name}-ce`
- [ ] Data Migration agent: copy `specs/{feature-name}-data-migration/` and run `/integration-review {feature-name}-data-migration`
- [ ] Reporting agent: copy `specs/{feature-name}-reporting/` and run `/integration-review {feature-name}-reporting`
- [ ] Ensure cross-cutting FR dependencies are reflected in all plans when `/integration-plan` is run
```
...
Integration Next Step    : /integration-review {feature-name}
CE/PA/F&O Next Step      : /integration-review {feature-name}-ce           ← run in the CE/PA/F&O agent
Data Migration Next Step : /integration-review {feature-name}-data-migration  ← run in the Data Migration agent
Reporting Next Step      : /integration-review {feature-name}-reporting    ← run in the Reporting agent
```
