---
mode: agent
description: "Implement an Azure Integration task card — generate C# Functions, Bicep, Logic App JSON. Triggers on: 'implement', 'generate code'."
---
...
3. If `validation-status` is not `READY TO IMPLEMENT`, stop immediately:
   - If `NEEDS REWORK`: "This task has not passed validation. Run `/integration-validate {feature-name}` first and fix all reported issues."
   ...
   - If field is missing: "This task has not been validated. Run `/integration-validate {feature-name}` before implementing."
