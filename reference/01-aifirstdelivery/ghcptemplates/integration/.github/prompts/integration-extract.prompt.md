---
mode: agent
description: "Extract Azure Integration test plan, suites, and cases to ALM-ready formats. Triggers on: 'extract', 'extract testplan'."
---

Extract test plan, suites, or individual test cases for an Integration feature into ALM-ready output files. Rich content (markdown, steps as structured arrays) is preserved intact in the JSON output — the primary ALM import artifact.

## Usage

```
/integration-extract testplan  {feature-name}
/integration-extract testsuites {feature-name} [suite-name]
/integration-extract testcases  {feature-name} [tc-id]
```

Read the first argument after `/integration-extract` to determine the sub-command, then follow the matching section.
...
1. Read `docs-generated/{feature-name}/test-plan-and-strategy.md`.
   ...
   - Parse each suite reference table (§3.1–§3.6): TC ID, ALM ID, and suite-specific columns.
   - Note the file link for each suite.

2. Read all six suite files and extract full content with rich formatting intact:
   ...
   Each file uses a markdown table. For each row extract all columns: TC ID, ALM ID, Interface, Message Type / Load / Method (suite-specific), FR Ref, Scenario, Steps, Expected Result, Priority, Environment, and any additional suite-specific columns.
   ...
   **Rich content rule:** Steps, Expected Result, and all multi-line fields must be extracted verbatim — do NOT flatten to a single line. Do NOT strip markdown syntax (`**`, `*`, `` ` ``, `-`, `|` tables, numbered lists, code blocks). Every `\n` in the source must survive unchanged.
   ...
   Parse the Steps cell content into a `{ step, action, expected }` array wherever the cell contains numbered step lines. If the cell is a narrative paragraph, wrap as a single-item array.
...
4. Write `docs-generated/{feature-name}/alm-extract/test-plan-extract.csv`.
   **Metadata summary only — no step content.** One row per test case. Columns:
   ```
   Domain,Feature,Suite ID,Suite Name,TC ID,ALM ID,Interface,FR Ref,Scenario,Priority,Environment,Context,Step Count
   ```
...
5. Write `docs-generated/{feature-name}/alm-extract/test-plan-extract.json`.
   **Primary ALM import artifact — full rich content preserved.** Structure:
   ```json
   {
     ...
     "test-suites": [
       {
         "suite-id": "TS-001",
         "suite-name": "Contract / Schema",
         "suite-file": "docs-generated/{feature-name}/test-cases/contract.md",
         "tc-count": 0,
         "test-cases": [
           {
             "tc-id": "TC-C001",
             ...
             "fr-ref": "FR-001",
             "message-type": "{context column value}",
             ...
             "fields": {
               ...
               "System.Description": "{interface + fr-ref + preconditions as markdown — ALM Agent converts to HTML}",
   ...
   **JSON generation rules:**
   ...
   - `steps` must be an array of `{ step, action, expected }` objects — never a flat string. If a cell contains a narrative paragraph, wrap as `[{ "step": 1, "action": "{full text}", "expected": "{expected result}" }]`.
...
6. Write `docs-generated/{feature-name}/alm-extract/test-plan-summary.md`.
   ...
   - Summary table: `TC ID | Suite | Interface | Scenario | Priority | Step Count | ALM ID`
...
7. Print the completion report:
   ...
   Feature      : {feature-name}
   Suites       : 6
     TS-001  Contract / Schema      : {N} TCs, {N} steps
     TS-002  Integration / E2E      : {N} TCs, {N} steps
     TS-003  Performance            : {N} TCs, {N} steps
     TS-004  Security               : {N} TCs, {N} steps
     TS-005  Chaos / Resilience     : {N} TCs, {N} steps
   ...
   Next step: /integration-alm sync-testplan {f}  (after creating test cases in ALM)
   ```
...
**suite-name options:** `contract` | `integration` | `performance` | `security` | `chaos` | `uat` | `all` (default: `all`)

### Steps

1. Determine which suite(s) to extract. If omitted or `all`, extract all six.

2. For each suite to extract, read the suite file:
   | Argument | File |
   |---|---|
   | `contract` | `docs-generated/{feature-name}/test-cases/contract.md` |
   | `integration` | `docs-generated/{feature-name}/test-cases/integration.md` |
   | `performance` | `docs-generated/{feature-name}/test-cases/performance.md` |
   | `security` | `docs-generated/{feature-name}/test-cases/security.md` |
   | `chaos` | `docs-generated/{feature-name}/test-cases/chaos.md` |
...
3. For each test case row, extract all columns with rich content intact. Parse the Steps cell into a `{ step, action, expected }` array. Preserve markdown in every text field — do not flatten multi-line content.
...
7. Write `docs-generated/{feature-name}/alm-extract/suites-summary.md`.
   One section per extracted suite: suite name, TC count, step count, and reference table `TC ID | Interface | Scenario | Priority | Step Count | ALM ID`.
...
**tc-id:** A single TC ID (e.g., `TC-C001`, `TC-I003`) or omit to extract every test case from all suites.
...
9. Print the completion report:
   ...
   Filter       : {tc-id | all}
   Test Cases   : {N}
     TC-C001   Contract / Schema   3 steps  ALM: 12345
     TC-I001   Integration / E2E   4 steps  ALM: —
     ...
   Total Steps  : {N}
