---
mode: agent
description: "Manage Azure DevOps work item synchronisation for Azure Integration. Triggers on: 'alm', 'sync work items'."
---
...
```
/integration-alm extract {feature-name}
/integration-alm sync    {feature-name} {uid} {alm-id}
/integration-alm sync    {feature-name} --file {path-to-alm-response.json}
/integration-alm get     {feature-name} {alm-id}
```

Read the first argument after `/integration-alm` to determine the sub-command, then follow the matching section.
...
5. Write to `output/{feature-name}/integration-alm/extract-{YYYYMMDD-HHmmss}.json`.
6. Print: item counts by level, output file path, and the instruction:
   ...
   Output   : output/{feature}/integration-alm/extract-{timestamp}.json
   Next step: Pass this file to the ALM Agent to create work items.
              The ALM Agent must call  /integration-alm sync {feature-name} --file {response.json}
...
**Single item (inline):**
```
/integration-alm sync account-loyalty-points account-loyalty-points-T-001 12345
```

**Bulk file** (sent by the ALM Agent after creating all work items):
```
/integration-alm sync account-loyalty-points --file output/account-loyalty-points/alm/alm-response.json
```
...
```
/integration-alm sync-testplan {feature-name}
/integration-alm sync-testplan {feature-name} {TC-ID} {ALM-ID}
```
...
Example: `/integration-alm sync-testplan my-feature TC-I001 12346`
...
- Record as "not found" if the TC ID does not appear in any table — do not fail.

**B. Update the suite file** — use the TC ID prefix to identify the target file:
| Prefix | File |
|---|---|
| TC-C… | `docs-generated/{feature-name}/test-cases/contract.md` |
| TC-I… | `docs-generated/{feature-name}/test-cases/integration.md` |
| TC-P… | `docs-generated/{feature-name}/test-cases/performance.md` |
| TC-SEC… | `docs-generated/{feature-name}/test-cases/security.md` |
| TC-CH… | `docs-generated/{feature-name}/test-cases/chaos.md` |
| TC-U… | `docs-generated/{feature-name}/test-cases/uat.md` |

- In the table row where TC ID matches, replace the ALM ID column value.
- If prefix is ambiguous or TC ID not found in expected file, scan all suite files.
...
Updated successfully:
  TC-C001   →  12345   test-plan ✓   contract.md ✓
  TC-I001   →  12346   test-plan ✓   integration.md ✓
  TC-SEC001 →  12350   test-plan ✓   security.md ✓

Not found (in mapping but not in files):
  {TC-ID}  — check TC ID spelling or re-run /integration-extract testplan to verify
...
```
/integration-alm get account-loyalty-points 10004
```
...
2. Find the item where `alm-id = {alm-id}`. If not found, stop:
   ...
   Run /integration-alm sync first to ensure ALM IDs have been written.
   ```
...
5. Write to `output/{feature-name}/integration-alm/get-{alm-id}-{YYYYMMDD}.json`.
6. Print:
...
Output    : output/{feature}/integration-alm/get-{alm-id}-{date}.json
```
