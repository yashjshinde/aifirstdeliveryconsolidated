---
mode: agent
description: "Review an Azure Integration spec for completeness and constitution compliance. Triggers on: 'review', 'spec review', 'approve spec'."
---
...
3. **Multi-Domain Detection — run first, before any other check.**
   ...
   - **BLOCKER text (CE / Power Apps):** "This spec contains D365 CE or Power Apps requirements that are out of scope for the Azure Integration agent. Run `/integration-split-spec {feature-name}` in the CE or Power Apps agent to separate the domains before sending the integration portion here."
   - **BLOCKER text (Data Migration):** "This spec contains Data Migration requirements (ADF pipelines, SFTP, staging) that belong to the Data Migration agent. Run `/integration-split-spec {feature-name}` in the originating domain agent to separate the integration and data migration portions before proceeding."
...
4b. **Cross-Feature Spec Scan** — informational only; does not block approval unless a hard CONFLICT is identified:
    ...
    - If a CONFLICT is detected (two features claiming incompatible modifications to the same component): raise as RECOMMENDED — "Component {name} is also claimed by feature {other-feature} with action {action}. Resolve the overlap before /integration-plan to avoid deployment conflicts."
...
|---|---|---|
| BLOCKER | Constitution violation or out-of-domain requirement | Sets NEEDS REWORK; blocks `/integration-plan` |
| REQUIRED | Missing information that prevents technical planning | Sets NEEDS REWORK; blocks `/integration-plan` |
...
- `APPROVED` — zero BLOCKERs, zero REQUIREDs. `/integration-fdd`, `/integration-testplan`, and `/integration-plan` may proceed.
- `NEEDS REWORK` — one or more BLOCKERs or REQUIREDs. `/integration-plan` refuses to proceed.
- `/integration-fdd` may be started in parallel with review if the spec is substantially complete, but must not be marked final until review is APPROVED.
...
Next step    : /integration-fdd {feature-name}

{If NEEDS REWORK:}
Output       : specs/{feature-name}/review.md
Resolve all BLOCKERs and REQUIREDs, then re-run /integration-review {feature-name}.
