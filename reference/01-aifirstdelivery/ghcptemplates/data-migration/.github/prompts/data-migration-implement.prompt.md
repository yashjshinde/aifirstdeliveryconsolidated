---
mode: agent
description: "Implement a Data Migration task card — generate ADF JSON, SQL DDL, and implementation record. Triggers on: 'implement', 'generate code', 'implement task'."
---
...
```
/data-migration-implement {migration-id} [{task-uid}]
```

Omit `{task-uid}` to implement all READY TO IMPLEMENT tasks.
Provide `{task-uid}` to implement a single task (e.g. `/data-migration-implement sftp-accounts T005`).
...
Next step: /data-migration-document {migration-id}
```
