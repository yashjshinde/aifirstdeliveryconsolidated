---
mode: agent
description: "Review a Data Migration plan for task readiness before task card generation. Triggers on: 'clarify', 'plan review', 'task readiness'."
---
...
```
/data-migration-clarify {migration-id}
```
...
**TASK-READY** — plan is complete and all tasks are well-defined. Proceed to /data-migration-tdd, /data-migration-blueprint, /data-migration-task.
...
{If TASK-READY:}
Next steps:
  /data-migration-tdd {migration-id}       → technical design document
  /data-migration-blueprint {migration-id} → solution blueprint
  /data-migration-task {migration-id}      → dev-ready task cards

{If CHANGES REQUIRED:}
Fix the listed issues and re-run /data-migration-clarify {migration-id}.
```

**GATE:** `/data-migration-tdd`, `/data-migration-blueprint`, and `/data-migration-task` may not run until this review is **TASK-READY**.
