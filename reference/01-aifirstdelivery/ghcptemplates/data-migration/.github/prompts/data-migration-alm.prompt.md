---
mode: agent
description: "Manage Azure DevOps work item synchronisation for Data Migration. Triggers on: 'alm', 'sync work items', 'alm extract'."
---
...
```
/data-migration-alm extract {migration-id}
/data-migration-alm sync {migration-id} {uid} {alm-id}
/data-migration-alm sync-all {migration-id}
/data-migration-alm get {migration-id} {alm-id}
/data-migration-alm push-tests {migration-id}
```
...
3. Produce `output/{migration-id}/data-migration-alm/extract-{timestamp}.json`:
...
════════════════════════════════════════
File   : output/{migration-id}/data-migration-alm/extract-{timestamp}.json
Items  : {N} (E:{N} F:{N} US:{N} T:{N})

Import this file into Azure DevOps, then use:
  /data-migration-alm sync {migration-id} {uid} {alm-id}   ← single item
  /data-migration-alm sync-all {migration-id}              ← bulk (requires alm-mapping.csv)
```
...
1. Read `output/{migration-id}/data-migration-alm/alm-mapping.csv` (format: `uid,alm_id`).
2. For each row, apply the same logic as `/data-migration-alm sync`.
...
2. Call MCP tool `ado_get_work_item`:
   - `id`: {alm-id}
3. Save response to `output/{migration-id}/data-migration-alm/get-{alm-id}-{date}.json`.
