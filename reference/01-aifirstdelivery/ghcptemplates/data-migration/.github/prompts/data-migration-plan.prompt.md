---
mode: agent
description: "Generate a Data Migration technical plan — work breakdown, task cards, work-items.yaml. Triggers on: 'plan', 'technical plan', 'break down spec'."
---
...
```
/data-migration-plan {migration-id}
```
...
`/data-migration-plan` must NOT create new L1/L2/L3 items — it generates **Task-level (L4) items only**,
...
in the ALM Traceability Matrix). `/data-migration-plan` identifies pending branches by reading the Source column
...
- uid: "{migration-id}-US-001"
  ...
  source: generated          # created by /data-migration-plan — ALM Agent will push this to ADO
  ...
  tasks:
    - uid: "{migration-id}-T-NNN"
      type: Task
      title: "..."
      source: generated-parent   # parent L3 was generated by /data-migration-plan, not from ALM
      estimate_hours: {N}
      tags: "{component-tag}"
      alm_id: null
      mr_refs:
...
> **Source:** Generated by `/data-migration-plan` — no L3 items were provided for parent {L2-ALM-ID}.
> The ALM Agent (`/wi-create-bulk`) will create this story in the ALM tool.

**Mapped MRs:** {MR-NNN if derivable from L2 scope, otherwise "— (to be assigned after /data-migration-review)"}
...
Next step: /data-migration-clarify {migration-id}
```
