---
mode: agent
description: "Generate a Data Migration spec from structured ALM work items (L1/L2/L3). Triggers on: 'spec-alm', 'structured intake', 'alm spec'."
---
...
```
/data-migration-spec-alm {migration-id}
```
...
2. Check `requirement-intake`. If it is not `structured`, stop:
   ...
   Use /data-migration-spec for plain-language requirements.
    ...
    - If `required`: every L2 in the input must have at least one L3 item. After parsing (step 5), stop if any L2 has no L3:
      ...
      Options:
        ...
        2. Set l3-intake: optional in constitution/10-alm-configuration.md
           to allow /data-migration-plan to generate missing L3 stories.
      ```
    - If `optional`: L3 may be absent or partially provided — continue. `/data-migration-plan` will generate L3 stories for any L2 with no coverage.
...
5. Parse the provided work items:
   ...
   - Preserve ALM IDs exactly as provided — they will be referenced by `/data-migration-plan`
   - **If `l3-intake: optional`:** For any L2 with no L3 items in the input, record it as a pending L2: set `source: pending` in the `alm-ids` YAML front matter and write `pending` as the Source column value in the Traceability Matrix. Do not stop — `/data-migration-plan` will decompose it into L3 stories.
...
7. For each L3 item, enhance the description:
   ...
   **If `l3-intake: optional` and a L2 has no L3 items (pending):**
   - Do NOT generate MR-NNN from the L2 description — MRs for this branch will be produced by `/data-migration-plan`
   - Write a pending placeholder block in §6 under the L2 heading:
     ...
     **Status:** No L3 items were provided for this L2 branch.
     `/data-migration-plan` will decompose {L2-ALM-ID} into {L3-Type} stories and generate Tasks.
     ```
...
10. Print summary:
    ...
    {L1-Type}    : {n}  (IDs: {list})
    {L2-Type}    : {n}  (IDs: {list})
    {L3-Type}    : {n} from ALM{, {n} pending — to be created by /data-migration-plan}
    MR-NNN       : {n} migration requirements generated
    Open Qs      : {n}
    Output       : specs/{migration-id}/spec.md

    Next step: /data-migration-review {migration-id}
    ```
...
- Every ALM-provided L3 (`source: alm`) must map to at least one MR-NNN in the ALM Traceability section — pending placeholders (`source: pending`) are exempt; MR-NNN will be generated by `/data-migration-plan`
...
- **If `l3-intake: optional`:** Do NOT create new L3 IDs in `/data-migration-spec-alm` — mark any missing L3 as pending: set `source: pending` in the YAML `alm-ids` front matter and `pending` in the Traceability Matrix Source column; `/data-migration-plan` will generate them
...
- Flag `intake: structured` in the YAML front matter — `/data-migration-plan` reads this to determine mode
- Flag `l3-intake: required` or `l3-intake: optional` in the YAML front matter — `/data-migration-plan` reads this to know whether to generate L3 stories for pending branches
