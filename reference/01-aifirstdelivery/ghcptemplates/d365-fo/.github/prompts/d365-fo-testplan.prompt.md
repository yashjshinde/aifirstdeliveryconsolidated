---
mode: agent
description: "Generate a D365 F&O test plan and test cases from an approved FDD. Triggers on: 'testplan', 'test plan', 'test strategy'."
---
...
```
/d365-fo-testplan {requirement-name}
```
...
Run /d365-fo-fdd-review {requirement-name} first.
```
...
Include an `ALM ID` column in every test case table, initialised to `*(pending — set by /d365-fo-alm sync)*` for all rows.
...
Next step  : /d365-fo-tdd {requirement-name}  (if not already run)
```
