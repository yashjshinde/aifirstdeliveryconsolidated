---
mode: agent
description: "Implement a single D365 F&O object from an approved plan — generate X++ code, unit tests, and implementation record. Triggers on: 'implement', 'generate code', 'implement object'."
---
...
```
/d365-fo-implement {requirement-name}/{Object-ID}
```

Examples:
```
/d365-fo-implement qms-validation/EXT-001
/d365-fo-implement qms-validation/DEN-042
/d365-fo-implement qms-validation/INT-022
```
...
Run /d365-fo-plan-review {requirement-name} first.
```
...
- If this object has a **SEQUENTIAL** dependency on another requirement's object: confirm the prerequisite object exists in `output/` or has `status: DONE` in its owning plan. If not, stop:
  ...
  Complete {other-requirement}/{other-object} first, then re-run /d365-fo-implement.
  ```
...
> **Note:** Test case documentation (TC-001, TC-002…) is created earlier by `/d365-fo-testplan`. The SysTestCase code generated here is the runnable implementation of the unit test scenarios from the test plan.
