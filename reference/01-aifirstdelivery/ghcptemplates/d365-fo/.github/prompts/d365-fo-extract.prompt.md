---
mode: agent
description: "Extract D365 F&O test plan, suites, and cases to ALM-ready CSV and JSON formats. Triggers on: 'extract', 'extract testplan', 'export test cases'."
---
...
```
/d365-fo-extract testplan  {requirement-name}
/d365-fo-extract testsuites {requirement-name} [suite-name]
/d365-fo-extract testcases  {requirement-name} [tc-id]
```

Read the first argument after `/d365-fo-extract` to determine the sub-command, then follow the matching section.
...
7. Print the completion report:
   ...
   Next step: /d365-fo-alm sync-testplan {req}  (after creating test cases in ALM)
   ```
