---
mode: agent
description: "Review a D365 F&O FDD for completeness, quality, and multi-domain detection. Triggers on: 'fdd-review', 'review fdd', 'approve fdd'."
---
...
```
/d365-fo-fdd-review {requirement-name}
```
...
Run /d365-fo-fdd {requirement-name} first.
```
...
- **BLOCKER text (Integration):** "This FDD contains Azure Integration requirements that are out of scope for the D365 F&O agent. Run `/d365-fo-split-spec {requirement-name}` to separate the F&O and integration portions before proceeding. The integration spec must then be handed to the Azure Integration agent."
- **BLOCKER text (Data Migration):** "This FDD contains Data Migration requirements (ADF pipelines, SFTP, staging) that are out of scope for the D365 F&O agent. Run `/d365-fo-split-spec {requirement-name}` to separate the F&O and data migration portions before proceeding. The data migration spec must then be handed to the Data Migration agent."
...
author: Claude Code (/d365-fo-fdd-review)
---
...
Next steps (can be run in parallel):
  /d365-fo-testplan {requirement-name}   ← generate test strategy and test cases from FDD
  /d365-fo-tdd {requirement-name}        ← generate IT System Design Specification
```

If NEEDS REWORK:
```
GATE BLOCKED — Fix all BLOCKERs in fdd.md and re-run /d365-fo-fdd-review.
```
