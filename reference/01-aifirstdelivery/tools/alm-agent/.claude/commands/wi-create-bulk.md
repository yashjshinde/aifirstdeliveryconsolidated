...
```
/wi-create-bulk {domain} {feature}
/wi-create-bulk {domain} {feature} --dry-run
```
...
Run /wi-create-bulk {domain} {feature} to create in ADO.
```
...
### 8. Auto-sync
Automatically run `/wi-sync {domain} {feature}` to write ALM IDs into all local documents.
