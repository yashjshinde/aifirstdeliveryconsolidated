...
- **Total items:** 33 (was 29 pre-2026-05-15-audit; +4 from DEFECT-005/006/007/008 → bk-030/031/032/033)
...
- **Partial (status updated 2026-05-15 audit):**
  - **bk-002** /split semantics — 5 of 6 thin spots covered in design/09 Pattern 2; multi-target collision-behaviour sub-detail still queued (reclassified from "queued" 2026-05-15 per DEFECT-004)
- **In progress:** 0
- **Queued (post-2026-05-15 audit) — coding follow-ons + content polish + new audit-driven items:**
  ...
  - **bk-029** Command-doc rebuild against design/17-command-doc-standard.md (88 bodies; per-agent unit) — DEFECT-002 follow-on
  - **bk-030** reporting agent design doc — deeper FDD/TDD content structure — **NEW 2026-05-15 (DEFECT-005)**
  - **bk-031** chat-ui SSE event-envelope contract — document the wire format in design/13 — **NEW 2026-05-15 (DEFECT-006)**
  - **bk-032** ALM round-trip Mermaid attachment strategy — detailed contract — **NEW 2026-05-15 (DEFECT-007)**
  - **bk-033** VS rates for the 8 reintroduced factors (R28 gap) — **NEW 2026-05-15 (DEFECT-008)**
- **Blocked / dropped:** 0

*Updated 2026-05-15 (post-audit, second pass): DEFECT-003 resolved in same session (`agents/integration/README.md` replaced from stub to full What/How/Details README — implementation log entry `2026-05-15-009`). DEFECT-004 reclassified bk-002 from "queued" to "partial 5/6". DEFECT-005/006/007/008 logged as new backlog items bk-030/031/032/033. See [../implementation.md `2026-05-15-009`](../implementation.md) for the audit + intake + resolution chain.*

*Updated 2026-05-15 post-Phase 10 close-out: all 9 implementation phases complete (Phases 0-9). 18 backlog items closed across the phases; the remaining items are coding follow-ons (MCP `brownfield-engine`, `brownfield_validators`, `alm_*`, `converters/`) and per-agent content polish. The platform v1 release-ready milestone is achieved at the structural / design level; per-agent body content and MCP-layer code are explicitly queued. See [../implementation.md § Phase 10](../implementation.md) and [verification-report-2026-05-15.md](verification-report-2026-05-15.md) for the close-out audit.*
...
| bk-002 | `/split` semantics — handoff manifest shape, target-agent feature folder init, multi-target, collision behavior | P3 | **partial (5 of 6 covered in design/09)** | [09-orchestration-patterns.md § Pattern 2](09-orchestration-patterns.md) | §14 Pattern 2 | **Status reclassified 2026-05-15** (DEFECT-004): design/09 Pattern 2 (lines 30-60) covers 5 of 6 thin spots: (1) handoff manifest schema `handoff.v1.json`, (2) skeleton-spec bootstrap path branching by docScope, (3) `.workflow.json` init with `phase: DEFINE`, `currentStates: [SPEC_DRAFT]`, `parent-handoff: {handoff-id}` frontmatter, (4) status flow PENDING/IN_PROGRESS/READY/CANCELLED + append-only history, (5) source-spec SPEC_SPLIT state marker (cross-ref in [04-workflow-gates.md](04-workflow-gates.md)). REMAINING 6th: **multi-target collision behaviour** (what happens when two split-emitted handoffs land in the same target agent for overlapping scope) — sub-detail; queued for a future ADR if/when collisions emerge in practice. |
...
| bk-029 | Command-doc rebuild: apply [design/17-command-doc-standard.md](17-command-doc-standard.md) to the 88 command bodies not yet rebuilt | P2 | queued | [17-command-doc-standard.md](17-command-doc-standard.md) | §25 (extension), DEFECT-002 | **Added 2026-05-15 from DEFECT-002.** Discoverability uplift across all 8 agents. Per-agent rebuild unit: solution-architect (3), d365-ce (17), d365-fo (19), integration (17), reporting (17), brownfield (9), alm (6). Total 88. `solution-estimate/.claude/commands/estimate.md` already rebuilt 2026-05-15 as the worked example. |
| bk-030 | reporting agent design doc — deeper FDD/TDD content structure | P3 | queued | [agents/reporting.md](agents/reporting.md) (108 lines; peers avg 178) | §7.4, §25, DEFECT-005 | **Added 2026-05-15 from DEFECT-005.** [`design/agents/reporting.md`](agents/reporting.md) is the thinnest per-agent design doc — Power BI / CE SSRS / paginated-report / RLS-OLS specifics could be detailed further (e.g., dataset model conventions, refresh strategy depth, theme JSON structure, sensitivity-label policy). Reporting agent was successfully built against the current doc in Phase 6 so this is a polish item, not a blocker. |
| bk-031 | chat-ui SSE event-envelope contract — document the wire format | P3 | queued | [13-chat-ui.md](13-chat-ui.md) | §20, DEFECT-006 | **Added 2026-05-15 from DEFECT-006.** [`design/13-chat-ui.md`](13-chat-ui.md) describes the chat-ui architecture + UX flows but does not document the SSE event envelope (event type names, payload shape per `stdout/stderr/exit/error`, `.workflow.json` change-event shape, error-event shape). The Phase 9 backend implements a working contract via `tools/chat-ui/backend/src/lib/cli-spawner.ts` + `routes/stream.ts`; the implementation is correct, but the wire format should be lifted into the design doc so future clients (CLI consumers, alternate frontends) can target it. |
| bk-032 | ALM round-trip Mermaid attachment strategy — detailed contract | P3 | queued | [agents/alm/constitution/02-alm-conventions.md § Mermaid handling](../agents/alm/constitution/02-alm-conventions.md), [design/agents/alm.md](agents/alm.md) | §14, §15, R14, DEFECT-007 | **Added 2026-05-15 from DEFECT-007.** ALM constitution describes "render PNG + preserve `<pre><code class='mermaid'>` source" but doesn't fully specify: (a) ADO attachment-size limits + the `60 MB` fallback path, (b) JIRA Cloud ADF attachment endpoint specifics vs JIRA Server wiki workaround, (c) `alm_roundtrip_check` diff algorithm + acceptable-loss threshold, (d) error handling when render fails. **Must precede `bk-018` converters/ MCP coding** to avoid downstream rework. |
| bk-033 | VS rates for the 8 reintroduced factors (R28 gap) | P3 | queued | [agents/solution-estimate/templates/factor-rates.yaml](../agents/solution-estimate/templates/factor-rates.yaml), [ADR-0009 § Negative consequences](adr/0009-solution-estimate-consolidated.md) | R28, DEFECT-008 | **Added 2026-05-15 from DEFECT-008.** 8 of the 103 factors are "reintroduced" beyond the canonical SI Excel (Azure Function Build & UT, Integration generic, CRM Master Data Prep, Model Driven App Changes, PCF Control Development, Excel Report, ExperLogix Report, Hierarchy Security). Their `VS` rate column is blank because R23 (4-level S/M/C/VC) was used to size them before R26 added the VS column. Per ADR-0009 § Negative consequences. Backfill rates from a tool-equivalent source. **Distinct from `bk-010`** which is about catalogue *extensions* (F&O / deeper Integration / deeper Reporting); bk-033 is about completing the *existing* 103. |
